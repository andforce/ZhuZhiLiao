...
- 可旋转、缩放和暂停自转的“哇声地球”，展示匿名粗略位置圆点与两分钟共鸣波纹
...
- 客户端：Swift 6、SwiftUI、Metal / MetalKit、Core Motion、Core Location、AVFoundation、Keychain
...
如需修改配置，可参考 `server/.env.example` 设置 `HOST`、`PORT`、`SQLITE_PATH` 和 `INITIAL_WAHS` 环境变量。客户端默认连接线上服务；要调试本地服务，请修改 `ZhuZhiLiao/Network/CounterService.swift` 中的 WebSocket 和 HTTP 端点常量。部署、认证和协议详情见 [`server/README.md`](server/README.md)。
...
动作数据仅在本机用于控制玩具，不会上传。在线服务会为安装创建随机匿名玩家 ID 和六位公开短码，并使用保存在 Keychain 中的令牌同步累计成绩；它们不是 iPhone 硬件标识符，也不包含姓名或联系方式。

哇声地球无需位置权限即可浏览。只有用户主动加入时，应用才请求“使用 App 期间”定位，并且只上传在客户端量化后的约 20 公里格网编号，不上传原始精确坐标，不进行后台定位。用户可单独退出哇声地球，也可在排行榜中清除匿名身份、排名、地球位置和本机个人累计；已计入的全球聚合总数不会回退。详见 [`submission/privacy-policy-zh-Hans.md`](submission/privacy-policy-zh-Hans.md)。
