...
<p align="center">
  <img src="iOS/screenshots/raw/zh-Hans/iphone69/02-自动演示.png" alt="自动演示" width="30%">
  <img src="iOS/screenshots/raw/zh-Hans/iphone69/03-手指控制.png" alt="手指控制" width="30%">
  <img src="iOS/screenshots/raw/zh-Hans/iphone69/01-安全提示.png" alt="安全提示" width="30%">
</p>

<p align="center">
  <img src="iOS/screenshots/readme/04-全球排行榜.png" alt="全球排行榜" width="30%">
  <img src="iOS/screenshots/readme/05-哇声地球.png" alt="哇声地球" width="30%">
</p>
...
1. 使用 Xcode 26 或兼容版本打开 `iOS/ZhuZhiLiao.xcodeproj`。
...
工程已提交 Xcode 项目文件，无需额外生成。如需根据 `iOS/project.yml` 重新生成，请先安装 [XcodeGen](https://github.com/yonaskolb/XcodeGen)，然后运行：
...
如需修改配置，可参考 `server/.env.example` 设置 `HOST`、`PORT`、`SQLITE_PATH` 和 `INITIAL_WAHS` 环境变量。客户端默认连接线上服务；要调试本地服务，请修改 `iOS/ZhuZhiLiao/Network/CounterService.swift` 或 `Android/app/src/main/java/com/azhegezhege/zhuzhiliao/network/CounterService.kt` 中的 WebSocket 和 HTTP 端点常量。部署、认证和协议详情见 [`server/README.md`](server/README.md)。
...
```bash
xcodebuild test \
  -project iOS/ZhuZhiLiao.xcodeproj \
  -scheme ZhuZhiLiao \
...
```text
iOS/ZhuZhiLiao/        iOS 应用源码与资源
iOS/ZhuZhiLiaoTests/   iOS 单元测试
iOS/.asc/              App Store Connect 与截图流水线配置
iOS/metadata/          App Store 元数据
iOS/screenshots/       App Store 截图与审核记录
iOS/submission/        隐私政策与提审材料
iOS/project.yml        XcodeGen 工程配置
...
哇声地球无需位置权限即可浏览。只有用户主动加入时，应用才请求“使用 App 期间”定位；Android 仅申请粗略位置。两端都只上传在客户端量化后的约 20 公里格网编号，不上传原始精确坐标，不进行后台定位。用户可单独退出哇声地球，也可在排行榜中清除匿名身份、排名、地球位置和本机个人累计；已计入的全球聚合总数不会回退。详见 [`iOS/submission/privacy-policy-zh-Hans.md`](iOS/submission/privacy-policy-zh-Hans.md)。
